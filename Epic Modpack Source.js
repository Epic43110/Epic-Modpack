...
function checkClient() {
    typeof Animal != 'undefined' && typeof vars != 'undefined' ? setTimeout(startFunctions, 100) : setTimeout(checkClient, 100);
}
checkClient();
...
function onClientLoad(func, customCondition) {
    // setTimeout(func, 1000);
    if (customCondition) customLoad.push([func, customCondition]);
    else onLoadFunctions.push(func);
}
...
function runCustom() {
    for (let [func, fcond] of customLoad) {
        var tempInterval = setInterval(function() {
            if (window[fcond]) clearInterval(tempInterval), setTimeout(func, 500);
        }, 5)
    }
}
...
onClientLoad(_ => {
    console.log("TEST TEST", window[drawGame]);
...
// setInterval(function() {
//     if (zoom_n < 24) zoom_n = 24;
//     else if (zoom_n > 26) zoom_n = 26;
// }, 5);
...
onClientLoad(function() {
    setTimeout(window[vars.main].toString()
        .replace('=0x37;_0x', '=0x37;updateLeaderBoard.apply(this, arguments);return;_0x')
    );
}, vars.main) //HERE TEST THIS
...
versionDisplayDiv.innerHTML = `<u>Epic Modpack LOCAL</u>`;
...
document.addEventListener('keydown', function(e) {
    ...
    switch (key) {
        ...
        case "Digit0": // Chat Hotkeys
            if (isTyping(e)) return;
            if (window[vars.aniChoice_isOpen]) {
